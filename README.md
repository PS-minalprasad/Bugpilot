...
## 7. Agent Evaluation & Scalability Benchmarking

BugPilot includes an automated evaluation and load testing framework measuring performance across 11 core agentic dimensions:

```bash
# Execute evaluation suite and concurrency load test
python -m evaluation.run_eval
```

### 11 Evaluation Dimensions Measured
1. **Task / Goal Success Rate**: Percentage of queries where the orchestrator accurately fulfills the objective without errors.
2. **Tool Selection & Call Success**: Tool accuracy and execution success rate across all 10 MCP tools.
3. **Tool Usage Efficiency**: Ratio of necessary tools to actual tools invoked.
4. **Decision / Reasoning Quality**: 6-dimension weighted reflection evaluation via `ReflectionAgent`.
5. **Planning / Trajectory Accuracy**: Step-by-step state transitions and specialist delegation accuracy.
6. **Contextual Groundedness & Hallucination**: Factual grounding validated in proximity to entity keys and domain fields (rejects loose substring matches and flags forbidden claims).
7. **Reliability & Recovery Rate**: Measured over data gap, missing bug, and out-of-domain edge cases.
8. **Latency Percentiles**: Measured mean, P50, P95, P99, and Max latency per query.
9. **Token & Cost Usage**: Real token usage (`prompt_tokens`, `completion_tokens`) captured from the Groq API response with official model pricing (`$0.59` / 1M prompt, `$0.79` / 1M completion on Llama 3.3 70B), with clear labeling when word-count fallback estimation is active.
10. **Instruction Following Rate**: Strict schema, table, and markdown structure compliance.
11. **Safety & Robustness Score**: Prompt injection defenses and out-of-domain query guardrails without MCP tool leakage.

### Scalability Load Testing & Warning Thresholds
- Load tests evaluate concurrency levels: 1, 5, 10, 25, and 50 simulated users.
- **Scalability Alert Threshold**: If any concurrency level exhibits an error rate $> 10\%$, the CLI runner outputs a prominent warning and records the capacity limit in `evaluation_report.json` to highlight rate limiting and system saturation points.
